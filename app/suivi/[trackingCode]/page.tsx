...
const STEPS = [
  { key: 'nouveau',      label: 'Commande reçue',     icon: '📥', desc: 'Votre commande a été enregistrée avec succès.' },
  { key: 'confirme',     label: 'Confirmée',           icon: '✅', desc: 'Notre équipe a validé votre commande.' },
  { key: 'en_livraison', label: 'En livraison',        icon: '🚚', desc: 'Votre colis est en route vers vous.' },
  { key: 'livre',        label: 'Livrée',              icon: '🎉', desc: 'Commande livrée. Merci pour votre achat !' },
];
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/commandes/tracking/${trackingCode}`)
      ...
      .then(r => {
        ...
        else setError('Code de suivi introuvable.');
      })
  ...
  return (
    <div className="min-h-screen" style={{ background: '#060D1F', color: '#E2E8F0' }}>
      ...
      <nav className="relative border-b px-6 py-4 flex items-center justify-between" style={{ borderColor: '#1E2D4A', background: '#060D1F/95' }}>
        ...
        <div className="text-xs text-gray-500 uppercase tracking-widest">Suivi de commande</div>
      </nav>
      ...
      <div className="relative max-w-2xl mx-auto px-4 py-12">
        ...
        {error && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-sm text-gray-500">Code recherché : <span className="font-mono text-cyan-400">{trackingCode}</span></p>
        ...
        {commande && (
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-black text-white mb-1">Commande {commande.reference}</h1>
              <p className="text-gray-400 text-sm">Code : <span className="font-mono text-cyan-400 font-bold">{commande.trackingCode}</span></p>
            </div>
            ...
            {!isCancelled ? (
              <div className="rounded-2xl p-6" style={{ background: '#0B1628', border: '1px solid #1E2D4A' }}>
                ...
                <div className="mt-6 p-4 rounded-xl text-center" style={{ background: '#060D1F', border: '1px solid #0099FF30' }}>
                  ...
                  {commande.status === 'en_livraison' && commande.clientQuartier && (
                    <div className="text-xs text-orange-400 mt-2">📍 Livraison vers : {commande.clientQuartier}</div>
                  )}
                </div>
              </div>
            ) : (
              <div className="rounded-2xl p-6 text-center" style={{ background: '#EF44441A', border: '1px solid #EF444440' }}>
                ...
                <div className="text-red-400 font-bold text-lg">Commande annulée</div>
                <div className="text-sm text-gray-400 mt-1">Cette commande a été annulée. Contactez-nous pour plus d&apos;informations.</div>
              </div>
            )}

            {/* Infos client */}
            <div className="rounded-2xl p-5" style={{ background: '#0B1628', border: '1px solid #1E2D4A' }}>
              <h3 className="text-xs text-gray-500 uppercase tracking-widest font-semibold mb-4">👤 Vos informations</h3>
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <div className="text-[10px] text-gray-500 uppercase mb-0.5">Paiement</div>
                  <div style={{ color: commande.paymentMode === 'online' ? '#00E5FF' : '#FF6B00' }}>
                    {commande.paymentMode === 'online' ? '💳 En ligne' : '🚚 À la livraison'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #1E2D4A' }}>
              <div className="px-5 py-3 border-b" style={{ background: '#0B1628', borderColor: '#1E2D4A' }}>
                <h3 className="text-xs text-gray-500 uppercase tracking-widest font-semibold">📦 Articles commandés</h3>
              </div>
              {commande.items.map((item, i) => (
                <div key={i} className="flex justify-between items-center px-5 py-3 border-b last:border-0 text-sm"
                  ...
                  <div>
                    ...
                    {item.digital && <div className="text-[10px] text-green-400">⚡ Livraison instantanée</div>}
                  </div>
            ...
            {commande.timeline && commande.timeline.length > 0 && (
              <div className="rounded-2xl p-5" style={{ background: '#0B1628', border: '1px solid #1E2D4A' }}>
                <h3 className="text-xs text-gray-500 uppercase tracking-widest font-semibold mb-4">📅 Historique</h3>
                    ...
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-gray-500">
                          {new Date(ev.date).toLocaleString('fr-FR')} · {ev.by}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="rounded-2xl p-5 text-center" style={{ background: '#0B1628', border: '1px solid #1E2D4A' }}>
              <p className="text-sm text-gray-400 mb-4">Une question sur votre commande ?</p>
